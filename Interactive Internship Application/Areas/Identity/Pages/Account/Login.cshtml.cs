        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // var user = await _signInManager.UserManager.GetUserAsync(HttpContext.User.Identity);
                    var user = await _signInManager.UserManager.FindByEmailAsync(Input.Email);
                    //below grabs the current logged in user's role, to then redirect to the appropriate landing page
                    var getCurrentRole = await _signInManager.UserManager.GetRolesAsync(user);

                    if(getCurrentRole[0] == "Admin")
                    {
                        return LocalRedirect("/DER");
                    }
                    else if (getCurrentRole[0] == "Professor")
                    {
